...
of the above methods, remember to (i) save the Org file where you
expect the auto-exporting to work, and (ii) revert the buffer (~M-x
revert-buffer~). That will ensure that the updated state of
=org-hugo-auto-export-mode= is effective.
#+end_note
